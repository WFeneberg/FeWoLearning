...
//
// Measured on this machine (Caliburn.Micro 5.0.258): ViewModelLocator.LocateForView
// constructs through IoC - this track's harness wires IoC.GetInstance to a fresh
// SimpleContainer per test, falling back to Activator.CreateInstance for anything
// unregistered - so calling it twice for an UNREGISTERED view model type gives two
// DIFFERENT instances (no caching of its own); register an instance in the container first
// and LocateForView returns THAT SAME instance instead of a new one. Unlike ex013's
// ViewLocator, which returns a placeholder TextBlock for a model it cannot find a view for,
// LocateForView returns a plain null for a view with no matching view model type at all -
// there is no placeholder object on this side.
