...
//
// Measured on this machine (Caliburn.Micro 5.0.258): a three-step sequence that completes
// normally runs every step, strictly in the order yielded - step 2 never starts before step 1's
// Completed has fired.
...
public class Ex042_CoroutineSequence
{
    ...
    /// <summary>The TODO: yield first, then second, then third - in that order, and nothing else.</summary>
    public static IEnumerable<IResult> Steps(IResult first, IResult second, IResult third) =>
        throw new NotImplementedException("TODO: Ex042 - yield first, then second, then third, in order");
}
