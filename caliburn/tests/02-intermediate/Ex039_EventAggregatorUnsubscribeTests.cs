...
public class Ex039_EventAggregatorUnsubscribeTests : CaliburnCoreContext
{
    ...
    public async Task A_Subscriber_With_No_Strong_Reference_Silently_Stops_Being_Delivered_To_After_Gc()
    {
        ...
        // an untouched stub: the tracked screen below is kept alive for the whole test, exactly
        // as every EventAggregator test in this track must (subscribers are held WEAKLY -
        // never subscribe an object created inline in a call argument).
        ...
        // against. [MethodImpl(NoInlining)] keeps the JIT from folding it into this frame and
        // extending the throwaway subscriber's lifetime past the call that creates it.
        var throwawayAggregator = new EventAggregator();
        SubscribeThrowaway(throwawayAggregator);
        ...
        static void SubscribeThrowaway(IEventAggregator aggregator) =>
            aggregator.Subscribe(new Ex039_ThrowawaySubscriber(), deliver => deliver());
    }
}
