...
public sealed class Ex094_FrameNavigator : IEx094_Navigator
{
    ...
    public bool Navigate(string route, object? parameter = null) =>
        // An unknown route is false, not an exception: a deep link or a stale button should
        // not take the app down.
        _routes.TryGetValue(route, out var pageType) && _frame.Navigate(pageType, parameter);
...
public sealed class Ex094_MenuViewModel
{
    ...
    // A route name and a parameter. No Frame, no page type, nothing that needs a UI to
    // exist - which is why this class is testable at all.
    public bool OpenDetail(int id) => _navigator.Navigate("detail", id);

    /// <summary>Goes back if it can.</summary>
    public bool Back() => _navigator.GoBack();
}
