...
public static class Ex084_RenderTransforms
{
    ...
    public static Transform CreateRotation(double degrees) => new RotateTransform { Angle = degrees };

    /// <summary>A scale about the origin.</summary>
    public static Transform CreateScale(double x, double y) => new ScaleTransform { ScaleX = x, ScaleY = y };

    /// <summary>
    /// A group that scales first and then rotates - which is the order the children appear
    /// in, not the reverse.
    /// </summary>
    public static Transform CreateScaleThenRotate(double scale, double degrees) => new TransformGroup
    {
        // First to last, so the scale happens before the rotation. Reversing the two lines
        // is a different transform, and a uniform scale hides the difference.
        Children =
        {
    ...
    public static Point Map(Transform transform, Point point) => transform.TransformPoint(point);

    /// <summary>
    /// The axis-aligned box containing <paramref name="rect"/> after
    /// <paramref name="transform"/>.
    /// </summary>
    // Not TransformPoint on a corner: this is the axis-aligned box *containing* the
    // transformed shape, which a 45-degree rotation makes bigger on both axes.
    public static Rect MapBounds(Transform transform, Rect rect) => transform.TransformBounds(rect);
}
