...
public partial class Ex080_CustomAutomationPeer : Control
{
    ...
    protected override AutomationPeer OnCreateAutomationPeer() => new Ex080_SwitchPeer(this);
}
...
public sealed class Ex080_SwitchPeer : FrameworkElementAutomationPeer, IInvokeProvider, IToggleProvider
{
    ...
    // Read from the control every time. A peer with its own copy announces the wrong state
    // the moment anything else changes the control.
    public ToggleState ToggleState => Switch.IsOn ? ToggleState.On : ToggleState.Off;

    /// <summary>The Invoke pattern: does what pressing the control does.</summary>
    public void Invoke() => Switch.Press();

    /// <summary>The Toggle pattern: moves to the next state, which here is the same press.</summary>
    // Two patterns over one behaviour: a screen reader may reach for either.
    public void Toggle() => Switch.Press();
    ...
    protected override string GetClassNameCore() => nameof(Ex080_CustomAutomationPeer);
}
