...
public sealed class Ex067_DebouncedInput
{
    ...
    public async Task ChangeAsync(string value)
    {
        ...
        if (previous is not null)
        {
            ...
            // the headless harness (see uno/README.md), and the synchronous form is the
            // more common idiom anyway.
            previous.Cancel();
            previous.Dispose();
        }
        ...
        try
        {
            await _delay(current.Token);
        }
        catch (OperationCanceledException)
        {
            // The normal path for every keystroke but the last one. Letting it out would
            // make the caller - a property setter, usually - handle an exception per
            // character typed.
            return;
        }
        ...
        Runs++;
        LastValue = value;
        await _action(value, current.Token);
    }
}
