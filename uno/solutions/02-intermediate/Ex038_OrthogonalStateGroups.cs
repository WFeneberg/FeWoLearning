...
public partial class Ex038_OrthogonalStateGroups : Control
{
    ...
    {
        // Both groups, every time, in a fixed order. GoToState picks the group by finding
        // which one declares the named state, so the two calls never interfere - and
        // updating only the group whose property moved loses the other one as soon as the
        // template is re-applied.
        Request(IsAvailable ? "Available" : "Unavailable");
        Request(IsChecked ? "Checked" : "Unchecked");
    }
