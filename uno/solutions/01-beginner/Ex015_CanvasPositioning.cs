...
public static class Ex015_CanvasPositioning
{
    ...
    public static Canvas CreateScene(FrameworkElement back, FrameworkElement front)
    {
        ...
        Canvas.SetLeft(back, 10);
        Canvas.SetTop(back, 20);
        Canvas.SetZIndex(back, 1);

        canvas.Children.Add(front);
        Canvas.SetLeft(front, 120);
        Canvas.SetTop(front, 40);

        // Setting this to 0 is not redundant in spirit even though 0 is the default: the
        // point of the pair is that paint order comes from ZIndex, and only falls back to
        // the Children order when the ZIndex values tie.
