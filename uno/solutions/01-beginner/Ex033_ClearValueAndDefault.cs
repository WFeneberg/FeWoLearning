...
public partial class Ex033_ClearValueAndDefault : DependencyObject
{
    ...
    public bool HasLocalQuantity =>
        // UnsetValue is a sentinel, not null: a property whose local value is genuinely
        // null still reads as "set", which is the distinction ClearValue undoes.
        ReadLocalValue(QuantityProperty) != DependencyProperty.UnsetValue;
    ...
    public void ResetQuantity() =>
        // Removes the local value only. Everything below it in the precedence order - a
        // style setter, an inherited value, the registered default - answers again, which
        // is what assigning 5 by hand would have blocked forever.
        ClearValue(QuantityProperty);
}
