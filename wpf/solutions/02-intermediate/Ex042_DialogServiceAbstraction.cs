// Exercise 042 - The dialog service seam (intermediate). REFERENCE SOLUTION.
...
/// <summary>
/// The real, production implementation - deliberately NOT exercised by any test in this row:
/// MessageBox.Show pumps its own modal message loop on the calling thread, so a test invoking
/// this on the harness's single STA dispatcher would hang the run rather than fail it. Testing
/// Ex042_ItemViewModel through Ex042_IDialogService instead - never through this class - is
/// exactly what the abstraction buys: full coverage of the decision logic below with no
/// window ever opening.
/// </summary>
...
public sealed class Ex042_ItemViewModel
{
    ...
    public bool Delete()
    {
        if (!_dialogService.Confirm($"Delete '{Name}'?"))
        {
            return false;
        }

        IsDeleted = true;
        _dialogService.Notify($"'{Name}' deleted.");
        return true;
    }
}
