// Exercise 067 - Global exception hooks on the dispatcher (intermediate).
...
public static class Ex067_GlobalExceptionHooks
{
    ...
    public static IDisposable Install(Dispatcher dispatcher, IList<Exception> log)
        => throw new NotImplementedException("TODO: Ex067 - subscribe a Dispatcher.UnhandledExceptionFilter handler that sets e.RequestCatch = false when e.Exception is Ex067_FatalException (leave RequestCatch alone otherwise); subscribe a Dispatcher.UnhandledException handler that adds e.Exception to `log` and sets e.Handled = true for everything the filter did not decline; return an IDisposable that unsubscribes both handlers from `dispatcher` when disposed");
}
