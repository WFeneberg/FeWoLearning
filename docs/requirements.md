...
| `kotlin/` | needs JDK 21 + Gradle installed (neither is present on this machine); the Kotlin compiler comes via the Gradle `kotlin("jvm")` plugin, no separate install needed beyond Gradle itself; no wrapper is committed — run `gradle wrapper` once you have Gradle, or invoke a system `gradle` directly |
...
`java/` and `kotlin/` are both fully seeded (100/100 exercises each: Gradle
build, stubs, JUnit 5 tests, reference solutions) but **completely
unverified** — this machine has neither a JDK, Gradle, nor Kotlin, so nothing
in either has ever been compiled. Expect real compile errors on the first
actual `gradle test` run in each; treat both as much higher-risk than every
other (verified) track until that first run happens.
