...
- **Verification (corrected 2026-09-05).** `dotnet test` works here **only when given `--solution` or `--project`** — the `Microsoft.Testing.Platform` front-end that `global.json` opts into takes those flags rather than a positional path, and with no argument it mis-resolves a `.slnx` directory to the built dll and reports "no tests were run" / exit 5 (identically for the pre-existing `wpf/` track). Working forms: `dotnet test --solution FeWoLearning.Security.slnx` and `dotnet test --project tests/FeWoLearning.Security.Tests.csproj`, with `--filter-class "*ExNNN*"` to scope. Building and running `security/tests/bin/Debug/net10.0-windows/FeWoLearning.Security.Tests.exe` directly also works, takes `-filter`, and is how every batch in this track was actually verified. (Spec §2.4.)
