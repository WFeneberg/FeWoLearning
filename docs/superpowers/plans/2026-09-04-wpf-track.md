...
- Every commit stages **explicit paths**. `git add -A` is forbidden: the working tree holds untracked `uno/` ex096–ex100 files and an untracked `docs/superpowers/specs/2026-09-04-caliburn-track-design.md` that must not be swept in.
...
- **WinForms interop.** `WindowsFormsHost` would pull WinForms into both content
  libraries for a single row. Rows 088 does `HwndSource`/`HwndHost` plus P/Invoke
...
`Unsubscribing_Stops_The_Notifications` is the one to check by eye: it asserts
`raised == 0`, which is also what a *do-nothing* implementation produces. It fails red
here only because the `add` accessor throws. Confirm that in the output — if it ever
passes against the stub, replace it with an assertion that the subscription worked
