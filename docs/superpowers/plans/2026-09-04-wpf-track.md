...
> **Superseded in one place.** The final whole-work review found that the harness member
> this plan calls `Host(...)` shadows `Microsoft.Extensions.Hosting.Host`, so
> `Host.CreateApplicationBuilder(...)` in any test deriving from `WpfTestContext` fails
> `CS0119` — which would have broken catalog rows 036–039, the exact rows
> `Microsoft.Extensions.Hosting` was added for. It was renamed to **`Show(...)`** (the name
> both `avalonia/` and `caliburn/` use for the same job) and now returns the `Window`
> rather than the element it was handed. Read `wpf/README.md` and
> `wpf/tests/_harness/WpfTestContext.cs` for the harness as built; the code blocks below
> are the plan as written, not the tree as it stands.
