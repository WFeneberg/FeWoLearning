...
public static class Ex032_DatabaseInitScripts
{
    ...
    {
        // The modern spelling: Aspire enumerates InitFolder now and copies each file
        // into /docker-entrypoint-initdb.d when the container is created. No mount, so
        // nothing depends on the daemon being able to see this machine's filesystem.
        var postgres = builder.AddPostgres("pg")
                              .WithInitFiles(InitFolder);

        // The database the consumer names. Its health, not the server's, is what
        // "the schema is there" is waited on through.
        var orders = postgres.AddDatabase("orders");

        // The same intent, the old way, kept side by side so the difference is
        // measurable rather than asserted from memory. WithInitBindMount is [Obsolete]
        // ("Use WithInitFiles instead"), and this track's solutions/ must build with
        // zero warnings, so the one call that needs it is bracketed rather than the
        // whole file being given a NoWarn.
#pragma warning disable CS0618 // WithInitBindMount is obsolete - that IS the exercise
        builder.AddPostgres("legacy")
               .WithInitBindMount(InitFolder);
#pragma warning restore CS0618

        // WithReference hands over the connection string; WaitFor is what keeps this
        // container from dialling a Postgres that is still running its init scripts.
        // Referencing without waiting is the bug the row exists to drill.
