...
public static class Ex020_RunVersusPublishMode
{
    public static void Configure(IDistributedApplicationBuilder builder)
    {
        ...
        {
            // A developer wants a database they can throw away. Aspire starts the
            // container, mints the password and hands the API the resolved string.
        ...
        {
            // Publishing must not describe a Postgres container - the database
            // already exists and is somebody else's to operate. There is no server
            // resource on this side at all, only the address of one.
            var orders = builder.AddConnectionString("orders", ReferenceExpression.Create(
                $"Host=orders.postgres.database.contoso.com;Port=5432;Database=orders;Username=app"));
