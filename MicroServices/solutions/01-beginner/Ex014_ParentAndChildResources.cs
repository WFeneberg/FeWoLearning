...
public static class Ex014_ParentAndChildResources
{
    ...
    {
        // Two databases on ONE server. Each AddDatabase call returns a separate
        // resource whose Parent is this very server object - which is what makes the
        // Assert.Same in the test meaningful rather than a type check in disguise.
        var pg = builder.AddPostgres("pg");
        pg.AddDatabase("billing");
        pg.AddDatabase("shipping");

        // Same mechanism, different flavour: the child's clause is appended at the
        // end of the parent's string here too, just spelt "Initial Catalog=".
        builder.AddSqlServer("sql")
               .AddDatabase("inventory");

        // Same mechanism again - and yet no interpolation. Mongo's database name goes
        // in the MIDDLE of a URI, so there is no tail to append and Aspire re-renders
        // host and port into the child's expression.
