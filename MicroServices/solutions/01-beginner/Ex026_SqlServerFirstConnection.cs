...
public static class Ex026_SqlServerFirstConnection
{
    ...
    {
        // The one credential SQL Server lets you choose. Declared as a secret
        // parameter rather than a literal, so it never appears in source and is
        // published as parameter.v0 with secret: true and no generated default -
        // the deployer supplies it.
        var saPassword = builder.AddParameter("sa-pw", secret: true);

        // AddSqlServer's parameter overload wires that same parameter into BOTH the
        // container's MSSQL_SA_PASSWORD and the connection string's Password= clause.
        // Without it Aspire generates a "sqldata-password" nobody chose.
        builder.AddSqlServer("sqldata", saPassword)
               .AddDatabase("catalog");
    }
}
