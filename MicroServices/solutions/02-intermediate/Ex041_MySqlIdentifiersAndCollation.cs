...
public static class Ex041_MySqlIdentifiersAndCollation
{
    ...
    public static void MapCatalog(ModelBuilder modelBuilder, ScriptTarget target, string tableName)
    {
        ...
        // MySQL stores each table as a file, and lower_case_table_names decides whether
        // the filesystem's case sensitivity leaks into SQL: 0 on Linux (it does), 1 on
        // Windows and macOS (it does not). Folding here is the only version of this
        // decision that behaves the same in both places.
        product.ToTable(target == ScriptTarget.MySql ? tableName.ToLowerInvariant() : tableName);

        product.Property(p => p.Name).HasMaxLength(255);
        product.Property(p => p.Sku).HasMaxLength(64);

        var descriptionIndex = product.HasIndex(p => p.Description).HasDatabaseName(DescriptionIndexName);

        if (target == ScriptTarget.MySql)
        {
            // Per COLUMN, not per model: UseCollation/HasCharSet on the ModelBuilder
            // describe CREATE DATABASE, which GenerateCreateScript never emits, so they
            // would leave no trace at all. Both calls are needed - a collation belongs to
            // a character set, and the pair is what "CHARACTER SET utf8mb4 COLLATE
            // utf8mb4_bin" in the DDL is made of.
            MySql.EntityFrameworkCore.Extensions.MySQLPropertyBuilderExtensions
                 .ForMySQLHasCharset(product.Property(p => p.Sku), CharSet);
            MySql.EntityFrameworkCore.Extensions.MySQLPropertyBuilderExtensions
                 .ForMySQLHasCollation(product.Property(p => p.Sku), CaseSensitiveCollation);

            // MySQL's text holds 64 KiB and lives outside the row; SQL Server's
            // nvarchar(max) is the nearest thing and is spelt nothing like it.
            product.Property(p => p.Description).HasColumnType("text");

            // InnoDB cannot index a TEXT column in full - the key would not fit - so the
            // index covers a prefix. Note (4) in the header: this provider records the
            // prefix and does not emit it, which is why the test reads the model.
            MySql.EntityFrameworkCore.Extensions.MySqlIndexBuilderExtensions
                 .HasPrefixLength(descriptionIndex, DescriptionIndexPrefixLength);
        }
        else
        {
            product.Property(p => p.Description).HasColumnType("nvarchar(max)");
        }
    }
    ...
    public static DbContextOptions<CatalogContext> CreateOptions(ScriptTarget target)
    {
        ...
        _ = target switch
        {
            ScriptTarget.MySql => options.UseMySQL("server=offline;database=catalog;user=root;password=none"),
            ScriptTarget.SqlServer => options.UseSqlServer("Server=offline;Database=catalog"),
    ...
    {
        // The MySQL flavour, not "some container": only AddMySql produces a
        // MySqlServerResource, whose expression fixes the login to root and keys Server
        // and Port separately - where SQL Server joins them with a comma and Oracle
        // writes "data source=host:port" in lower case.
        builder.AddMySql(ServerResourceName)
