...
public static class Ex039_SeedDataInTheModel
{
    ...
    {
        // HasData is a MODEL statement, not code that runs at boot: EF folds it into the
        // migration as INSERTs, and a database created from this model already holds the
        // rows. That is why the ids are explicit - EF has to know them at design time to
        // decide, on the next migration, whether a row was added, changed or removed.
        modelBuilder.Entity<Category>().HasData(SeededCategories);
    }
    ...
    /// </summary>
    public static async Task<IReadOnlyList<string>> SeedSettingsAsync(
        CatalogContext context, CancellationToken cancellationToken)
    {
        // Read only the keys this seed owns. Loading the whole table and rewriting it is
        // the mutant that deletes somebody else's row.
        var owned = StartupSettings.Keys.ToArray();
        var existing = await context.Settings
            .Where(setting => owned.Contains(setting.Key))
        ...
        foreach (var (key, value) in StartupSettings)
        {
            ...
            {
                // The "up" half of upsert: a value an operator edited is restored, which
                // is what separates this from "insert if the table is empty".
                row.Value = value;
            }
            else
            {
