...
public static class Ex036_EfCoreAgainstSqlServer
{
    ...
    public static void Configure(IDistributedApplicationBuilder builder)
        => throw new NotImplementedException(
            "TODO: ex036 - add a SQL Server \"sqldata\" carrying a database whose resource "
    ...
    public static void AddCatalogDbContext(IHostApplicationBuilder builder)
        => throw new NotImplementedException(
            "TODO: ex036 - register CatalogContext with UseSqlServer over the injected "
            + "ConnectionStrings:catalog. No fallback: if the key is missing, the context "
            + "must end up with no connection string rather than with localhost.");
}
