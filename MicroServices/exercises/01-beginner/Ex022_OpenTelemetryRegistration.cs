...
public static class Ex022_OpenTelemetryRegistration
{
    ...
    /// <summary>
    /// TODO: ex022 - register the ORDERS source with the tracer provider and the ORDERS
    /// meter with the meter provider, and nothing else. Name them exactly; a wildcard
    /// sweeps in the Legacy pair above and fails this row. Return the builder.
    /// </summary>
    public static IHostApplicationBuilder AddOrdersTelemetry(this IHostApplicationBuilder builder)
        => throw new NotImplementedException(
            "TODO: ex022 - AddOpenTelemetry() with WithTracing(t => t.AddSource("
            + "OrdersSourceName)) and WithMetrics(m => m.AddMeter(OrdersSourceName)). "
            + "Do NOT use a wildcard, and do NOT register the Legacy pair.");
}
