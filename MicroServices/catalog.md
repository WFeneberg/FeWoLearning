...
| 031 | DataVolumesPerFlavour | `WithDataVolume` vs `WithDataBindMount`, and that the container path is flavour-specific — one shared constant is wrong. **Corrected 2026-09-07**: the Postgres path this row originally quoted, `/var/lib/postgresql/data`, is the **17-and-earlier** one; on 13.5.3's default tag (18.3) `WithDataVolume` lands on `/var/lib/postgresql`, because it reads the configured image tag. So the flavours are `/var/lib/postgresql` (pg 18) or `/var/lib/postgresql/data` (pg 17), `/data/db`, `/var/opt/mssql` — and even ONE flavour has two answers, which the exercise grades with two Postgres servers on different tags. Two further measured claims the row was widened to cover, because the original spec graded neither: `WithVolume(name, target)` writes a mount **byte-identical** to `WithDataVolume(name)`, so the helper is provable only through the ANONYMOUS overload's generated volume name; and SQL Server's `WithDataBindMount` writes **three** mounts where its `WithDataVolume` writes one | ✅ |
...
| 041 | MySqlIdentifiersAndCollation | `AddMySql`, `MySqlDatabaseResource` and its own expression; then the schema differences — `utf8mb4` collation and table-name case sensitivity read off the generated DDL, and the index prefix length read off the EF model, because `MySql.EntityFrameworkCore` 10.0.9 STORES `HasPrefixLength` and does not emit it (measured; the generated `CREATE INDEX` carries no `(32)`, which InnoDB would reject on a TEXT column) | ✅ |
