...
public static class Ex042_OtlpExporterConfiguration
{
    ...
    public static OtlpExporterOptions FromEnvironment() =>
        // The constructor reads the environment. Nothing else is needed, and that is the
        // design: the application never mentions where its telemetry goes.
        new();
    ...
    public static IReadOnlyDictionary<string, string> ParseHeaders(string? headersVariable)
    {
        ...
        if (string.IsNullOrWhiteSpace(headersVariable)) return headers;

        foreach (var pair in headersVariable.Split(',', StringSplitOptions.RemoveEmptyEntries))
        {
    ...
    public static TracerProvider BuildOtlp(Uri tracesEndpoint, string serviceName) =>
        Sdk.CreateTracerProviderBuilder()
            ...
            .AddOtlpExporter(options =>
            {
                ...
                // Simple, not batch: a batch processor would still be waiting when the
                // test looks, and the row is about the wire rather than about buffering.
