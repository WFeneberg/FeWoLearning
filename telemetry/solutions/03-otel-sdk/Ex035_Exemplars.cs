...
public static class Ex035_Exemplars
{
    ...
    private static readonly Histogram<double> Duration =
        Meter.CreateHistogram<double>(InstrumentName, "ms");
    ...
    public static MeterProvider BuildMetrics(ICollection<Metric> exported, ExemplarFilterType filter) =>
        Sdk.CreateMeterProviderBuilder()
            .AddMeter(MeterName)
            // Off by default. Exemplars are a deliberate choice, and their value is
            // bounded by the sampling rate the tracer provider was given.
    ...
    /// </summary>
    public static void RecordDuration(double milliseconds) => Duration.Record(milliseconds);
    ...
    public static string? InsideASpan(Action work)
    {
        ...
        // The measurement must happen while the span is CURRENT: the exemplar filter
        // reads Activity.Current at Record time, so work done after the span closed
        // carries nothing.
        work();
