...
public static class Ex036_OtelLogsPipeline
{
    ...
    public static ILoggerFactory Build(
        ...
        LoggerFactory.Create(builder => builder
            ...
            {
                // Off by default, and deliberately so: rendering is an allocation per
                // record that nothing downstream needs, since a backend can render it
                // from Body and Attributes whenever a human looks.
    ...
    public static void LogShipped(ILogger logger, string orderId, string city) =>
        // A constant template, exactly as in row 001 - and here you can see where it
        // lands: the template becomes Body, the arguments become Attributes.
        logger.LogInformation(Shipped, Template, orderId, city);
}
