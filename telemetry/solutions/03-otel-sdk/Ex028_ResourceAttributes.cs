...
public static class Ex028_ResourceAttributes
{
    ...
    public static Resource BuildResource(string instanceId) =>
        // CreateEmpty, not CreateDefault: the default already carries the SDK's guesses,
        // and starting from it would make it impossible to tell what this method
        ...
        ResourceBuilder.CreateEmpty()
            ...
            // MERGES with what is already there rather than replacing it, which is what
            // lets an operator add deployment.environment without erasing the service's
            // own identity.
    ...
    public static TracerProvider Build(ICollection<Activity> exported, string instanceId) =>
        Sdk.CreateTracerProviderBuilder()
            ...
            .SetResourceBuilder(
                ResourceBuilder.CreateEmpty().AddAttributes(BuildResource(instanceId).Attributes))
