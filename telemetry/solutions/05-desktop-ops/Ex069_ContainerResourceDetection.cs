...
public static class Ex069_ContainerResourceDetection
{
    ...
    {
        // v1 first, because when it says something it is the more direct answer.
        if (cgroupContents is not null)
        {
            var v1 = CgroupId.Match(cgroupContents);
            if (v1.Success) return v1.Value;
        }

        // Under v2 the file above reads "0::/" and the match failed, so the id has to come
        // from the one place that still names it.
        if (mountInfoContents is not null)
        {
            var v2 = MountInfoId.Match(mountInfoContents);
            if (v2.Success) return v2.Groups["id"].Value;
        }

        // Null, and the caller must leave the attribute off entirely. See BuildResource.
        return null;
    }

    // The lookarounds matter: without them a 70-character hex path segment would match its
    // first 64 characters and yield an id that belongs to nothing.
    private static readonly Regex CgroupId = new(
        "(?<![0-9a-f])[0-9a-f]{64}(?![0-9a-f])",
        RegexOptions.Compiled | RegexOptions.CultureInvariant);

    private static readonly Regex MountInfoId = new(
        "/containers/(?<id>[0-9a-f]{64})",
        RegexOptions.Compiled | RegexOptions.CultureInvariant);
    ...
    public static Resource BuildResource(string? cgroupContents, string? mountInfoContents)
    {
        ...
        if (!string.IsNullOrEmpty(serviceName)) attributes[ServiceNameAttribute] = serviceName;

        var containerId = DetectContainerId(cgroupContents, mountInfoContents);

        // The if is the exercise. Writing "unknown" here instead would make every desktop
        // installation in the fleet look like one very busy container called unknown -
        // "unknown" is a value: it groups, it charts, and it lies.
        if (containerId is not null) attributes[ContainerIdAttribute] = containerId;
        ...
        string level, string messageTemplate, IReadOnlyDictionary<string, object?> fields)
    {
        ...
        // Indented: false is not a default worth relying on by accident - a log driver reads
        // stdout line by line, so a record spread over several lines becomes several broken
        // records.
        using (var writer = new Utf8JsonWriter(buffer, new JsonWriterOptions { Indented = false }))
        {
            ...
            // Unrendered. The template is the thing ten million records group by; the
            // rendered sentence is ten million distinct strings.
            writer.WriteString("template", messageTemplate);

            foreach (var (name, value) in fields)
            {
                writer.WritePropertyName(name);

                // Serialized by its own type, so a number stays a number. A field that
                // arrives as a string cannot be summed, ranged or plotted by whatever
                // receives it.
