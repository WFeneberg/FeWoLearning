...
public static class Ex063_StartupPerformanceTracing
{
    ...
    /// </summary>
    public static async Task RunStartupAsync(IReadOnlyList<(string Name, Func<Task> Work)> phases)
    {
        using var startup = Source.StartActivity(StartupSpanName);

        foreach (var (name, work) in phases)
        {
            // Declared inside the loop, so each phase is a SIBLING under the root rather
            // than a link in a chain that claims dependencies nobody has.
            using var phase = Source.StartActivity(name);
            phase?.SetTag(PhaseTag, name);
            ...
            {
                phase?.AddException(failure);
                phase?.SetStatus(ActivityStatusCode.Error, failure.Message);
                startup?.SetStatus(ActivityStatusCode.Error, failure.Message);

                // Nothing after this runs, and the caller finds out. A startup that
                // carries on past a broken phase opens into a half-initialised
                // application.
                throw;
            }
        }
