...
public static class Ex059_DispatcherLatencyMetric
{
    ...
    private static readonly Histogram<double> Latency = Meter.CreateHistogram<double>(
        LatencyHistogram, LatencyUnit, "How long work waited in the dispatcher queue.");

    public static async Task PostMeasuredAsync(
        Dispatcher dispatcher, DispatcherPriority priority, Action work)
    {
        // Taken HERE, at the moment the work is queued. Anything later measures something
        // else.
        ...
        await dispatcher.InvokeAsync(
            () =>
            {
                // And stopped the instant the work BEGINS, before it runs. The wait and the
                // work are different numbers with different causes and different fixes.
                Latency.Record(
                    Stopwatch.GetElapsedTime(queued).TotalMilliseconds,
                    new KeyValuePair<string, object?>(PriorityTag, priority.ToString()));
