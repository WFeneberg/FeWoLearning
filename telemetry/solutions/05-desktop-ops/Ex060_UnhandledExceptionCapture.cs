...
public static class Ex060_UnhandledExceptionCapture
{
    ...
    /// thing.
    /// </summary>
    // Keyed by INSTANCE, and holding its keys weakly so a reported exception does not
    // outlive the incident. "Report only the first" would lose every later failure;
    // "report only one per message" would merge separate incidents that happen to say the
    // same thing. Object identity is the only thing that means "this one, arriving again".
    ...
    public static void Capture(ILogger logger, Exception failure, string source)
    {
        lock (Reported)
        {
            if (Reported.TryGetValue(failure, out _)) return;

            Reported.Add(failure, new object());
        }

        logger.LogCritical(failure, Template, source);
    }
    ...
    public static void AttachTo(Dispatcher dispatcher, ILogger logger) =>
        ...
        {
            Capture(logger, e.Exception, DispatcherSource);

            // Without this the process goes down. Setting it is a decision to keep running
            // with state you no longer trust, which is why it belongs at the top level and
            // nowhere else.
            e.Handled = true;
        };
}
