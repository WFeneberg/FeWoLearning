...
public sealed class Ex064_OfflineBufferAndReplay : BaseExporter<Activity>
{
    ...
    private readonly List<string> delivered = [];
    ...
    {
        // Copied out of the batch first: a Batch<T> is only valid for the duration of this
        // call, so anything buffered has to be a value rather than a reference into it.
        var names = new List<string>();
        foreach (var activity in batch) names.Add(activity.DisplayName);
        ...
        lock (delivered)
        {
            if (!IsOnline)
            {
                ...
                {
                    // The OLDEST goes. The newest records are the ones nearest to whatever
                    // is going wrong now, which is the opposite of a queue's instinct.
            ...
            // Replayed FIRST, so the past does not arrive after the present.
            while (buffer.Count > 0) delivered.Add(buffer.Dequeue());

            delivered.AddRange(names);
        }
