...
public static class Ex007_ExceptionLogging
{
    ...
    public static void LogImportFailure(ILogger logger, string file, Exception error) =>
        // The exception goes in the exception slot, where the pipeline can keep the
        // whole object - type, stack and the inner chain. The message stays a constant
        // template so every import failure groups as one event.
        logger.LogError(error, "Import of {File} failed", file);
}
