...
public static class Ex024_MeterListenerLifecycle
{
    ...
    public static MeterListener CreateListener(string meterName, Action<string, long> onMeasurement)
    {
        ...
        {
            // Fires once per instrument, for instruments that already exist AND for
            // every one created later. Not a census at startup.
            InstrumentPublished = (instrument, self) =>
            {
                if (instrument.Meter.Name != meterName) return;

                // Being published is not being subscribed. Opt in, per instrument.
                self.EnableMeasurementEvents(instrument);
            },
        };

        listener.SetMeasurementEventCallback<long>(
            (instrument, value, _, _) => onMeasurement(instrument.Name, value));

        // Mandatory, and easy to leave out because everything above it looks complete.
        listener.Start();
