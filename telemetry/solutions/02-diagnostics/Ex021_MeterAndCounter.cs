...
public static class Ex021_MeterAndCounter
{
    ...
    /// </summary>
    // Created ONCE, alongside the meter. An instrument built per call would publish
    // itself to every listener again and again, and the "exactly one instrument" fact
    // is what catches it.
    private static readonly Counter<long> Processed =
        Meter.CreateCounter<long>(InstrumentName, InstrumentUnit);

    public static void RecordProcessed(string outcome, string region) =>
        Processed.Add(
            1,
            new KeyValuePair<string, object?>(OutcomeTag, outcome),
            new KeyValuePair<string, object?>(RegionTag, region));
}
