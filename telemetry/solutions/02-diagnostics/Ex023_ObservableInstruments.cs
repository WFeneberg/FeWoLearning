...
public static class Ex023_ObservableInstruments
{
    ...
    /// </summary>
    private static readonly Counter<long> Granted =
        Meter.CreateCounter<long>(LeasesGrantedCounter, "{lease}");

    private static readonly UpDownCounter<long> Active =
        Meter.CreateUpDownCounter<long>(LeasesActiveUpDown, "{lease}");

    private static readonly Lock GaugeLock = new();
    private static Func<int>? depthSource;
    private static ObservableGauge<int>? depthGauge;

    public static void RegisterQueueDepth(Func<int> readDepth)
    {
        lock (GaugeLock)
        {
            depthSource = readDepth;

            // At most once, ever. An instrument cannot be removed from a Meter, so a
            // second gauge under the same name would leave both alive and every
            // collection would report the value twice - which reads as a doubled queue
            // rather than as a bug.
            //
            // The callback closes over the FIELD, not over the parameter, so a later
            // call swaps the source without creating anything.
            depthGauge ??= Meter.CreateObservableGauge(
                QueueDepthGauge,
                () => depthSource?.Invoke() ?? 0,
                "{item}");
        }
    }
    ...
    public static void ReleaseLease() =>
        // Only the up-down counter moves. Subtracting from the monotonic counter would
        // not read as "minus one" downstream - it reads as a process restart, and the
        // whole delta gets discarded.
        Active.Add(-1);
}
