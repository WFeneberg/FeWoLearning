...
public static class Ex017_TagsBaggageEvents
{
    ...
    {
        using var request = Source.StartActivity(RequestName);

        // A tag belongs to THIS activity and goes no further.
        request?.SetTag(RegionTagKey, region);

        // Baggage rides down the whole subtree - and, with a propagator, across
        // process boundaries. Deliberately NOT also set as a tag: doing both would
        // make every fact pass while hiding which mechanism did the work.
        request?.AddBaggage(TenantBaggageKey, tenantId);

        using var handler = Source.StartActivity(HandlerName);

        for (var attempt = 1; attempt <= retries; attempt++)
        {
            // An event, not a tag: "this happened, n times, at these moments". A tag
            // could only say "n" and would lose the when.
            handler?.AddEvent(new ActivityEvent(
                RetryEventName,
                tags: new ActivityTagsCollection { { AttemptTagKey, attempt } }));
        }
    }
}
