...
public static class Ex015_ActivitySourceAndListener
{
    ...
    {
        // `using` stops the activity at the end of the method and restores whatever
        // Activity.Current was before it. Returning it afterwards is fine - a stopped
        // activity still carries everything it recorded.
        using var activity = Source.StartActivity(name);

        // The null-conditional is what makes this safe with nobody listening, and it
        // is also what makes the silence invisible. Both halves are worth seeing.
        activity?.SetTag(ItemCountTag, itemCount);
    ...
    public static ActivityListener CreateListener(Action<Activity> onStopped) =>
        new()
        {
            // Scoped to one source. A listener that returns true for everything
            // receives the framework's activities, other libraries' and other
            // components' as well, and whatever consumes it then reports numbers that
            // have nothing to do with this code.
            ShouldListenTo = source => source.Name == SourceName,

            // AllData, not PropagationData. PropagationData produces a real, healthy
            // looking activity whose IsAllDataRequested is false - which does not stop
            // SetTag from writing (measured), but does tell every well-behaved caller
            // that the detail is unwanted, and does make an SDK downstream drop the
            // activity entirely.
