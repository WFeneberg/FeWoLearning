...
public static class Ex054_ErrorStatusAndProblemDetails
{
    ...
    {
        endpoints.MapGet(FailingRoute, string () =>
            throw new InvalidOperationException(SecretExceptionMessage));

        endpoints.MapGet(WorkingRoute, (string id) => $"order {id}");
    }
    ...
    public static async Task HandleAsync(HttpContext context, RequestDelegate next, ILogger logger)
    {
        ...
        {
            // The span the instrumentation opened is still current here, which is what
            // makes all three signals name the same trace without any of them being told.
            Activity.Current?.AddException(failure);
            Activity.Current?.SetStatus(ActivityStatusCode.Error, failure.Message);

            // The exception as the exception argument - row 007 - so the type, the stack
            // and the inner chain survive.
            logger.LogError(failure, "Request to {Path} failed", context.Request.Path.Value);

            context.Response.StatusCode = StatusCodes.Status500InternalServerError;

            // The contentType argument is load-bearing: WriteAsJsonAsync OVERWRITES
            // Response.ContentType with application/json otherwise, so setting it
            // beforehand is silently undone.
            //
            // The body says that it failed and nothing about why. Everything else is in
            // the telemetry, which is behind your authentication rather than in front.
