...
public static class Ex047_HttpClientPropagation
{
    ...
    /// listening.
    /// </summary>
    public static DelegatingHandler CreatePropagatingHandler() => new PropagatingHandler();

    private sealed class PropagatingHandler : DelegatingHandler
    {
        protected override async Task<HttpResponseMessage> SendAsync(
            HttpRequestMessage request, CancellationToken cancellationToken)
        {
            using var activity = Source.StartActivity(ClientSpanName, ActivityKind.Client);

            // ITS OWN context, not Activity.Current's parent and not whatever was ambient.
            // Injecting the ambient one attaches the remote span a level too high, so the
            // client and server spans become siblings and the network hop disappears.
            ...
            // Forwarded whether or not anything was listening: activity is null when
            // nothing is, and the request still has to go out.
    ...
    {
        var parent = headers.TryGetValue(TraceParentHeader, out var traceParent)
                     && ActivityContext.TryParse(traceParent, null, isRemote: true, out var context)
            ? context
            : default;

        using var activity = Source.StartActivity(ServerSpanName, ActivityKind.Server, parent);
