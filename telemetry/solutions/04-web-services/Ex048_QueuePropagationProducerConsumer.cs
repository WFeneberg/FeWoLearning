...
public static class Ex048_QueuePropagationProducerConsumer
{
    ...
    public static QueueMessage Publish(string body)
    {
        ...
        using var activity = Source.StartActivity($"{QueueName} publish", ActivityKind.Producer);
        Describe(activity);
    ...
    {
        // No header is not an error. A message already in the queue before the
        // propagation shipped, or one from a producer you do not own, gets a fresh trace -
        // an unlinked trace is a gap in a picture, a throwing consumer is a poison message
        // that blocks the queue.
        var parent = message.Headers.TryGetValue(TraceParentHeader, out var traceParent)
                     && ActivityContext.TryParse(traceParent, null, isRemote: true, out var context)
            ? context
            : default;

        using var activity = Source.StartActivity(
            $"{QueueName} process", ActivityKind.Consumer, parent);
    ...
    private static void Describe(Activity? activity)
    {
        activity?.SetTag(MessagingSystemAttribute, MessagingSystem);
        activity?.SetTag(MessagingDestinationAttribute, QueueName);
    }
}
