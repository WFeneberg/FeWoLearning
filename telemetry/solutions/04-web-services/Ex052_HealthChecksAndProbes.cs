...
public static class Ex052_HealthChecksAndProbes
{
    ...
    public static void ConfigureHealthChecks(IServiceCollection services) =>
        services.AddHealthChecks()
            // Touches nothing it does not own. If a check can fail for a reason a restart
            // cannot fix, it is not a liveness check.
            .AddCheck(SelfCheckName, () => HealthCheckResult.Healthy("running"), [LiveTag])
            .AddCheck(DatabaseCheckName, DatabaseResult, [ReadyTag]);
    ...
    {
        // One registry, two filtered views of it - rather than two hand-maintained lists
        // that drift the first time somebody adds a check and forgets which endpoint
        // should run it.
