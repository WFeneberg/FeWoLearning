...
public static class Ex055_HttpRequestLogging
{
    ...
    {
        // Started BEFORE next, which is the whole of the second clause. Starting it after
        // measures the time to write a response header.
        ...
        finally
        {
            ...
            // Read by NAME from the allowlist. Nothing else in the query is looked at,
            // let alone recorded.
            var page = context.Request.Query.TryGetValue(SafeQueryParameter, out var value)
                ? value.ToString()
                : null;
