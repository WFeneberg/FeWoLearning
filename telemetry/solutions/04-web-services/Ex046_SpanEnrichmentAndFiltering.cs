...
public static class Ex046_SpanEnrichmentAndFiltering
{
    ...
    public static void ConfigureTracing(IServiceCollection services, ICollection<Activity> exported) =>
        services.AddOpenTelemetry().WithTracing(tracing => tracing
            ...
            {
                // Runs BEFORE the activity exists, so a filtered request costs a
                // predicate rather than a span that is built, serialised and then thrown
                // away somewhere downstream.
                options.Filter = context =>
                    !context.Request.Path.StartsWithSegments(HealthRoute);

                // Reads the request itself, so two requests get two answers. No header,
                // no attribute - rather than an empty string nobody can filter out.
                options.EnrichWithHttpRequest = (activity, request) =>
                {
                    if (request.Headers.TryGetValue(TenantHeader, out var tenant))
                    {
                        activity.SetTag(TenantAttribute, tenant.ToString());
                    }
                };
            })
    ...
    public static void MapEndpoints(IEndpointRouteBuilder endpoints)
    {
        ...
        // Still served, still answering, still failing loudly when it fails. What the
        // filter removes is the record of it having been boring.
        endpoints.MapGet(HealthRoute, () => "healthy");
    }
}
