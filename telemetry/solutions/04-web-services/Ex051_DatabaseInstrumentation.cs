...
public static class Ex051_DatabaseInstrumentation
{
    ...
    {
        using var activity = Source.StartActivity($"{operation} {system}", ActivityKind.Client);

        activity?.SetTag(DbSystemAttribute, system);

        // CommandText, and nothing derived from command.Parameters. The statement still
        // carries its placeholders, which is both the safe thing and the useful thing:
        // one query text that ran ten million times rather than ten million texts.
        activity?.SetTag(DbQueryTextAttribute, command.CommandText);
