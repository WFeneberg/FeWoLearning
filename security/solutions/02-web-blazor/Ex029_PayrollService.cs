...
// Exercise 029 — ClientAuthIsNotEnforcement (reference solution).
...
public sealed class Ex029_PayrollService
{
    public bool TryApprove(ClaimsPrincipal caller, int requestId, out string? denial)
    {
        ...
        {
            denial = "caller is not authenticated";
            return false;
        }

        if (!caller.IsInRole("approver"))
        {
            denial = "caller does not hold the approver role";
            return false;
        }
