//! Exercise 081 — Lock-free increments with `AtomicUsize` (advanced).
//! Goal:   a counter shared across threads with no `Mutex` at all, using an
//!         atomic read-modify-write instead, and proving it's genuinely
//!         race-free by checking every returned "previous value" is unique.
//! Drills: `AtomicUsize::fetch_add`, `Ordering`, lock-free vs. lock-based
//!         sharing, deterministic concurrency assertions via `join()`.
...
impl AtomicCounter {
    ...
    /// Atomically adds one and returns the value from *before* the add.
    pub fn increment(&self) -> usize {
        todo!("self.value.fetch_add(1, Ordering::SeqCst)")
    }

    pub fn get(&self) -> usize {
        todo!("self.value.load(Ordering::SeqCst)")
    }
}

/// Spawns `thread_count` threads, each calling `increment()` exactly
/// `times_per_thread` times, and returns every "previous value" `increment`
/// returned, across all threads, after joining them all.
