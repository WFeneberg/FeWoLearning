...
impl<T> Arena<T> {
    ...
    pub fn insert(&mut self, value: T) -> Handle<T> {
        todo!(
            "if free_list has an index, overwrite that slot with Occupied at its \
             current generation and return a Handle to it; otherwise push a new \
             Occupied slot at generation 0 and return a Handle to the new index"
        )
    }
    ...
    pub fn get(&self, handle: Handle<T>) -> Option<&T> {
        todo!("look up slots[handle.index] (bounds-checked); return Some(&value) only if Occupied with a matching generation")
    }

    /// Mutable counterpart to [`Arena::get`].
    pub fn get_mut(&mut self, handle: Handle<T>) -> Option<&mut T> {
        todo!("same lookup as `get`, but returning `&mut T`")
    }
    ...
    pub fn remove(&mut self, handle: Handle<T>) -> Option<T> {
        todo!(
            "if the handle is live, replace its slot with Vacant {{ generation: generation + 1 }}, \
             push its index onto free_list, and return the extracted value; otherwise None"
        )
    }
}
