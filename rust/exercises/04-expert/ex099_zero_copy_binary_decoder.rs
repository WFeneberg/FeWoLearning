...
pub fn decode_records(buf: &[u8]) -> Result<Vec<Record<'_>>, DecodeError> {
    todo!(
        "loop while there is unread input left (offset < buf.len()): if fewer than 3 \
         bytes remain, Err(DecodeError {{ offset, message: \"...\" }}) (truncated header); \
         otherwise read 1 type byte + 2 length bytes (u16::from_le_bytes) at `offset`; if \
         `offset + 3 + length` exceeds buf.len(), Err (declared length exceeds buffer); \
         otherwise push Record {{ record_type, value: &buf[value_start..value_end] }} \
         (a borrowed sub-slice — never copy) and advance offset past it"
    )
}
