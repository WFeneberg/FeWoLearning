//! Exercise 040 — Implementing a trait for your own types (intermediate).
//! Goal:   implement the same trait for two unrelated structs, each with its
//!         own formula.
//! Drills: `impl Trait for Type`, one trait shared by multiple implementors.
...
impl Area for Circle {
    fn area(&self) -> f64 {
        todo!("Circle::area for radius {}", self.radius)
    }
}

impl Area for Rectangle {
    fn area(&self) -> f64 {
        todo!("Rectangle::area for {}x{}", self.width, self.height)
    }
}
